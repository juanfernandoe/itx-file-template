...
class ParamSelectView extends View
  ...
  onConfirm: ->
    ...
    if cfg["classname"]
      cfg["classpath"] = cfg["classname"]
      cfg["classnamelower"] = cfg["classname"].toLowerCase()
      cfg["classnameroute"] = cfg["classname"].replace(/([A-Z])/g,'-$1').toLowerCase().replace("_-","/").replace(/^-/,"");
      #cfg["classNameRoute"] = cfg["className"].replace(//,)

    if cfg["namespace"] && cfg["classname"]
      cfg["classpath"] = cfg["namespace"]+"_"+cfg["classname"]
      cfg["classnameroute"] = cfg["classpath"].replace(/([A-Z])/g,'-$1').toLowerCase().replace("_-","/").replace(/^-/,"");
    ...
    for rule in ((@template.rules?(cfg).items) ? [])
      ...
      if rule.sourceTemplateFile
        ...
        if cfg["classname"]
          fullPathToNewFile = fullPathToNewFile.replace("{{classname}}",cfg["classname"])
